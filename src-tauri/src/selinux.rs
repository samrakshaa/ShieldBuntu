...
pub async fn apply_firewall_rules() -> Result<String, String> {
    ...
    let script_path = current_dir.join("scripts/apply/selinux.sh");
    let log_file_path = current_dir.join("logs/selinux_log.txt");
...
pub async fn reverse_firewall_rules() -> Result<String, String> {
    let password = get_password().ok_or_else(|| "Password not available".to_string())?;
    let current_dir = std::env::current_dir().map_err(|e| format!("Error getting current directory: {}", e))?;
    let script_path = current_dir.join("scripts/reverse/r-selinux.sh");
    let log_file_path = current_dir.join("logs/reverse_selinux_log.txt");
...
pub async fn check_firewall() -> Result<String, String> {
    let password = get_password().ok_or_else(|| "Password not available".to_string())?;
    let current_dir = std::env::current_dir().map_err(|e| format!("Error getting current directory: {}", e))?;
    let script_path = current_dir.join("scripts/check/check_selinux.sh");
