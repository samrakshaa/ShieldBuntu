...
#[tauri::command]
pub async fn no_exec(handle: tauri::AppHandle) -> Result<String, String> {
    let password = get_password().ok_or_else(|| "Password not available".to_string())?;
    let script_path = handle
        .path_resolver()
        .resolve_resource("scripts/apply/no_exec.sh")
    ...
    let log_file_path = Path::new(&log_directory).join("no_exec.txt");
